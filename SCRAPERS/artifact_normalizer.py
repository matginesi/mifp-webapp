...
Usage:
    python SCRAPERS/artifact_normalizer.py \\
...
ALLOWED_STATUSES = {'published', 'draft', 'review', 'quarantined', 'duplicate'}
...
def _normalize_review_status(value) -> str:
    status = _clean(value or 'published').lower()
    if status in {'needs_review', 'pending', 'candidate', 'archived'}:
        status = 'review'
    return status if status in ALLOWED_STATUSES else 'published'
...
def normalize_events(input_dirs: list[Path]) -> list[dict]:
    ...
    for r in deduped:
        ...
        review_status = _normalize_review_status(r.get('review_status'))
...
def normalize_news(input_dirs: list[Path]) -> list[dict]:
    ...
    for r in deduped:
        ...
        status = _normalize_review_status(r.get('review_status'))
...
def _publication_page_matches(record: dict) -> bool:
    section = str(record.get('configured_section') or '').casefold()
    kind = str(record.get('configured_kind') or '').casefold()
    url = _clean(record.get('url') or record.get('link') or record.get('source_url') or '').casefold()
    return (
        section == 'publications'
        or kind == 'publications'
        or 'research-mifp-publications' in url
        or 'research-mifp-pubblications' in url
        or '/research/publications/' in url
        or '/research/pubblications/' in url
    )


def _publication_titles_from_page(record: dict, text: str) -> list[str]:
    generic = {
        'mifp publications', 'mifp pubblications', "members' publications",
        "members' pubblications", 'publications', 'pubblications', 'authors',
        'research', 'download', 'scarica',
    }
    candidates = []
    for raw in record.get('headings') or []:
        title = _clean(raw)
        key = title.casefold()
        if not title or key in generic or len(title) < 8:
            continue
        if title not in text:
            continue
        candidates.append(title)
    # Preserve page order and collapse duplicated headings.
    seen = set()
    ordered = []
    for title in sorted(candidates, key=lambda value: text.find(value)):
        key = _norm_key(title)
...
def _publication_pdf_assets(record: dict) -> list[dict]:
    assets = []
    for raw in record.get('assets') or []:
        if not isinstance(raw, dict):
            continue
        url = _clean(raw.get('download_url') or raw.get('url') or '')
        ext = _clean(raw.get('extension') or '').casefold()
        kind = _clean(raw.get('kind') or '').casefold()
        if url.casefold().split('?', 1)[0].endswith('.pdf') or ext == '.pdf' or kind == 'pdf':
            item = dict(raw)
            if item.get('local_path') and not item.get('path'):
                item['path'] = item['local_path']
            assets.append(item)
    return assets


def _publication_from_block(title: str, block: str, url: str, pdf_asset: dict | None = None) -> OrderedDict:
    authors = ''
    abstract = block
    author_match = re.search(r'\bAuthors?\b\s*(.*?)(?=\b(?:Download|Scarica|Prenota)\b|$)', block, re.IGNORECASE)
    if author_match:
        authors = _clean(author_match.group(1).strip(' -:;,.'))
        abstract = _clean(block[:author_match.start()].strip(' -:;,.'))
    else:
        # Legacy mirrors often encode publications as:
        #   File: ... Uploaded: ... Downloads: N Rossi M., Smith J. This paper ...
        # Preserve the older builder's author extraction for that format while
        # keeping the richer catalogue/heading parser above.
        after_downloads = re.sub(
            r'^.*?\bDownloads:\s*\d+\s*', '', block, count=1, flags=re.IGNORECASE
        )
        if after_downloads != block:
            after_downloads = _clean(after_downloads)
            marker = re.search(r'\s+(The|This|We|Here|Our|In|A|An|It)\s+', after_downloads)
            if marker:
                authors = _clean(after_downloads[:marker.start()].rstrip('.,;:'))
                abstract = _clean(after_downloads[marker.start():])
            else:
                authors = _clean(after_downloads[:240].rstrip('.,;:'))
                abstract = _clean(after_downloads[240:])

    year = None
    uploaded = re.search(r'\bUploaded:\s*(\d{2})\.(\d{2})\.(\d{2,4})\b', block, re.IGNORECASE)
    if uploaded:
        year = int(uploaded.group(3))
        if year < 100:
    ...
    if year is None:
        ...
        if ym:
            year = int(ym.group(0))
    pdf_url = _clean((pdf_asset or {}).get('download_url') or (pdf_asset or {}).get('url') or '')
    documents = [pdf_asset] if pdf_asset else []
    return OrderedDict([
        ('type', 'publication'),
        ('title', title),
        ('year', year),
...
def normalize_publications(input_dirs: list[Path]) -> list[dict]:
    pages = [r for r in _iter_legacy_records(input_dirs, 'pages_all.jsonl') if _publication_page_matches(r)]
    out = []
    for r in pages:
        ...
        # The historical MIFP page is a catalogue: many H2 publication entries
        # live in one HTML page. Split that catalogue using the scraped heading
        # order instead of inventing one publication from the page URL.
        titles = _publication_titles_from_page(r, text)
        pdf_assets = _publication_pdf_assets(r)
        if titles:
            positions = [(title, text.find(title)) for title in titles]
            positions = [(title, pos) for title, pos in positions if pos >= 0]
            positions.sort(key=lambda item: item[1])
            for index, (title, pos) in enumerate(positions):
                end = positions[index + 1][1] if index + 1 < len(positions) else len(text)
                block = _clean(text[pos + len(title):end])
                pdf_asset = pdf_assets[index] if index < len(pdf_assets) else None
                out.append(_publication_from_block(title, block, url, pdf_asset))
            continue

        # Compatibility fallback for mirrors where one publication is stored
        # per page/file.
        title_match = re.search(r'\bFile:\s*(.+?)\s+Uploaded:', text, re.IGNORECASE)
        if title_match:
        ...
        if not title or title.casefold() in {'mifp publications', 'mifp pubblications', "members' publications", 'publications', 'pubblications', 'atom', 'rss', 'privacy policy'}:
            continue
        out.append(_publication_from_block(title, text, url, pdf_assets[0] if pdf_assets else None))

    # Dedupe duplicate headings/pages but keep the richest instance.
    return list(_dedup_by_best(out, lambda r: _slugify(r.get('title') or '') + ':' + str(r.get('year') or ''), lambda r: len(_clean(r.get('abstract'))) + 200 * len(r.get('documents') or [])))
...
def normalize_research_areas(input_dirs: list[Path]) -> list[dict]:
    # The old site has used several slugs (notably /research-research1), so
    # identify the actual research page by section + content rather than one
    # hard-coded URL/title spelling.
    candidates = []
    for r in _iter_legacy_records(input_dirs, 'pages_all.jsonl'):
        section = str(r.get('configured_section') or '').casefold()
        kind = str(r.get('configured_kind') or '').casefold()
        title = str(r.get('configured_title') or r.get('title') or '').strip().casefold()
        url = _clean(r.get('url') or r.get('link') or r.get('source_url') or '')
        lower_url = url.casefold()
        text = _clean(r.get('text') or r.get('body') or r.get('content') or '')
        if not text:
            continue
        if any(marker in lower_url for marker in ('mifp-pub', '/publications', '/pubblications', 'research-projects', 'presentations')):
            continue
        needle_hits = sum(1 for _, needle in RESEARCH_AREA_DEFS if needle.casefold() in text.casefold())
        looks_like_research = (
            (section == 'research' and kind in {'research', ''})
            or 'research-research' in lower_url
            or title == 'research'
        )
        if looks_like_research and needle_hits:
            candidates.append((needle_hits, len(text), url, text))
    if not candidates:
        return []

    _hits, _length, source_url, research_text = max(candidates, key=lambda item: (item[0], item[1]))
    out = []
    lower_text = research_text.casefold()
    for sort_order, (area_title, needle) in enumerate(RESEARCH_AREA_DEFS, start=1):
        pos = lower_text.find(needle.casefold())
        if pos < 0:
            continue
