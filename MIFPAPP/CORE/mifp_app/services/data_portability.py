...
def bundle_to_jsonl_file(
    ...
    *,
    ...
    """Write canonical record-only JSONL directly to disk.

    JSONL is deliberately the lightweight data-interchange format: one
    canonical record per line, with no embedded binary assets and no
    installation-owned ``state.json`` equivalent. Use the ZIP export when
    files or durable dashboard state must travel with the records.

    ``assets_dir`` and ``app_version`` remain in the signature because ZIP and
    JSONL exporters share the same service API; JSONL itself contains records only.
    """
    del assets_dir, app_version
    ...
    records = _records_for_scope(conn, scope)
    counts = _record_counts(records)
    report("Writing canonical JSONL…", 25)
    ...
    with temporary.open("wb") as output:
        ...
        for index, record in enumerate(records, 1):
            ...
            if index == len(records) or index % 250 == 0:
                report("Writing canonical JSONL…", 25 + (70 * index // total))
...
def import_jsonl_payload(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("JSONL import is not available") from exc
    if size > Config.IMPORT_MAX_JSONL_BYTES:
        raise ValueError(f"JSONL import exceeds maximum size: {Config.IMPORT_MAX_JSONL_BYTES} bytes")
    return import_jsonl(
        conn, path, dry_run=dry_run, assets_dir=assets_dir, progress=progress,
        asset_detail=asset_detail, force_import=force_import, source_name=source_name,
        cancel_check=cancel_check, commit=commit, file_session=file_session,
    )
...
def import_zip_payload(
    ...
    *,
    ...
    owned_file_session = file_session is None
    file_session = file_session or AssetWriteSession(assets_dir)
    try:
        ...
        if owned_file_session and not dry_run:
...
def _restore_durable_state(
    ...
    *,
    ...
    _restore_unlinked_assets(
        conn, state.get("assets") or [], assets_dir, packaged_assets_dir, restored,
        file_session=file_session,
    )
    return restored
    ...
    *,
    ...
    for item in assets:
        ...
        if source.is_file():
            ...
            if target.is_file() and sha256_file(target) != source_checksum:
                target, path_text = conflict_safe_asset_target(assets_dir, path_text, source_checksum)
            if not target.is_file():
                writer = file_session or AssetWriteSession(assets_dir)
                writer.install(source, target)
...
def _validate_manifest_object(manifest: Any) -> dict[str, Any]:
    ...
    if package_format == CONTENT_FORMAT and format_version != CONTENT_FORMAT_VERSION:
        raise ValueError(
            f"{CONTENT_FORMAT} packages must declare format_version={CONTENT_FORMAT_VERSION}"
        )
    ...
    if package_format in {CANONICAL_FORMAT, CONTENT_FORMAT} and not records_sha256:
        raise ValueError(f"{package_format} packages require records_sha256")
