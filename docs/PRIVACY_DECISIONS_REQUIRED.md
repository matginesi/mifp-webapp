...
## 1. Legal basis for membership applications — OPEN

`Privacy.md` §6 states consent (Article 6(1)(a) GDPR) for the membership
application and the steps needed to enter into membership. That wording is the
position the policy carried before this cleanup and it was deliberately **not**
changed, because choosing a legal basis is a controller decision.

Decide and record:

- the single primary basis for reviewing a membership application, and what
  happens to the application if consent is refused or withdrawn;
- whether consent is the right basis at all, or whether the controller considers
  the processing necessary for the performance of a contract / pre-contractual
  steps (Article 6(1)(b));
- whether any separate, genuinely optional purpose exists that would justify a
  consent checkbox. **Do not add a mandatory "I consent" checkbox** to the form
  unless such a purpose exists;
- the retention period that follows from the chosen basis.

Evidence the review keeps: the application itself, the reviewer's decision, and
...
1. open `Dashboard → Privacy & Cookies`;
...
| Consent banner | Removed: there are no optional cookies, so no consent mechanism is needed and none is shown. |
