...
Prima di creare lo ZIP, l'export della dashboard prova anche a **materializzare temporaneamente tutti gli asset remoti tracciati nel database**, indipendentemente dal dominio pubblico di origine. Il matcher di preservazione predefinito è `*`; `PORTABLE_EXPORT_PRESERVE_DOMAINS` può restringerlo esplicitamente se un'installazione lo richiede. Questa regola riguarda solo `assets`: i normali URL in `entity_links` restano link e non vengono scandagliati o copiati. Le risposte HTML vengono rifiutate dal downloader degli asset, così una normale pagina web non viene salvata per errore come documento. I file recuperabili vengono scaricati in staging e inseriti nello ZIP senza modificare il database o la libreria asset live. Il manifest contiene un oggetto `preservation` con conteggi, eventuali errori e `remaining_remote`; gli ZIP v2 più vecchi senza questo oggetto restano validi.

Il re-import di soli ZIP dashboard `mifp-jsonl-v2` è volutamente **offline**: ripristina i file presenti nel package e non lancia la recovery HTTP post-import. Se un asset remoto non è stato materializzato durante l'export, resta esplicitamente irrisolto invece di rendere il restore dipendente dalla disponibilità del sito sorgente. JSONL e package scraper `mifp-content` mantengono invece il normale comportamento di ingest/recovery degli asset remoti.
